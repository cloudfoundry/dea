...
module DEA
  ...
  class Agent
    ...
    def send_heartbeat
      ...
      heartbeat = {:droplets => [], :dea => VCAP::Component.uuid, :tier => @tier }
    ...
    def process_dea_discover(message, reply)
      ...
      if @shutting_down
        ...
        #Ensure app's tier matches dea's tier
        unless message_json['tier'] == @tier
          @logger.debug("Ignoring request, app_tier=#{message_json['tier']} doesn't match dea_tier=#{@tier}.")
          return
        end
